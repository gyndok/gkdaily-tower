...
| `factory.py` | P4 Script Factory (failover-only): weekdays, if no special-edition script is in Drive by 08:05 CT (config.factory.failover_at), picks the top uncovered topic and writes the script with the factory model (Sonnet 5 since the 2026-09-07 cost audit) + web search, delivering to `GK Daily/scripts/` for the untouched producer. Runs under the podcasts venv (needs `anthropic`). `--plan` / `--run [--stage]`. |
